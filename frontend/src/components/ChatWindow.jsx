...
const ChatWindow = ({ selectedUser, onBack }) => {
  ...
  if (!selectedUser) {
    return (
      <div className="flex-1 flex flex-col items-center justify-center bg-chat-panel">
        <div className="text-center">
          ...
          <h2 className="text-xl font-bold text-chat-text mb-2">Welcome to ChatApp</h2>
          <p className="text-chat-muted text-sm max-w-xs">
  ...
  return (
    <div className="flex flex-col w-full h-full bg-chat-bg overflow-hidden">

      {/* Header */}
      <div className="px-4 lg:px-6 py-4 border-b border-chat-border bg-chat-panel flex items-center gap-3 shrink-0">
        
        {/* Back button — mobile only */}
        <button
          onClick={onBack}
          className="p-2 cursor-pointer -ml-1 rounded-full hover:bg-chat-surface transition-colors"
        >
          <svg className="w-5 h-5 text-chat-text" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        <div className="relative">
          <img
            ...
            className="w-10 h-10 rounded-full bg-chat-surface border border-chat-border"
          />
          {isOnline && (
        ...
        <div className="flex-1">
          <h2 className="font-semibold text-chat-text">{selectedUser.name}</h2>
      ...
      {/* Input */}
      <div className="px-4 lg:px-6 py-4 border-t border-chat-border bg-chat-panel shrink-0">
        ...
        <p className="text-xs text-chat-muted text-center mt-2">
